...
//            tile.ParseFromString(mvt);
            std::fstream input(mRootDir + "/mvt/b.mvt", std::ios::in | std::ios::binary);
            ...
            auto mesh = shapeFactory.polygon(polygons);
            auto material = mEngine.getResourceManager().acquireMaterial("building");
            auto building = std::make_shared<Entity>(mesh, material, 0.0f, 10.0f, 0.0f);
